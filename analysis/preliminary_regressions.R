...
t1 <- glm(any_tighten ~ finstress_qt_mean + executive_election_4qt +
              country + year + quarter
          , data = dem, family = 'binomial')

t2 <- glm(any_tighten ~ finstress_qt_mean + cbi +
              country + year + quarter
          , data = dem, family = 'binomial')

t3 <- glm(any_tighten ~ finstress_qt_mean*cbi +
              country + year + quarter
          , data = dem, family = 'binomial')

t4 <- glm(any_tighten ~ finstress_qt_mean + cbi + execrlc +
              country + year + quarter
          , data = dem, family = 'binomial')

t5 <- glm(any_tighten ~ finstress_qt_mean + cbi +  fiscal_trans_gfs +
              country + year + quarter
          , data = dem, family = 'binomial')

t6 <- glm(any_tighten ~ finstress_qt_mean + cbi + domestic_credit_change +
              country + year + quarter
          , data = dem, family = 'binomial')

t7 <- glm(any_tighten ~ finstress_qt_mean + cbi + gdp_growth +
              country + year + quarter
          , data = dem, family = 'binomial')

t8 <- glm(any_tighten ~ finstress_qt_mean + cbi + inflation +
              country + year + quarter
          , data = dem, family = 'binomial')
